#ifdef      iarduino_I2C_h															//	Если код данного файла уже включён в скетч.
	#ifndef iarduino_I2C_SW															//	Если не выбран программный (SoftWare) алгоритм реализации шины I2C
	#ifndef iarduino_I2C_HW															//	Если не выбран аппаратный  (HardWare) алгоритм реализации шины I2C
	#ifndef iarduino_I2C_TW															//	Если не выбран алгоритм реализации шины I2C через библиотеку Wire.h (TwoWire)
		#error У вас установлены устаревшие библиотеки iarduino, пожалуйста обновите все библиотеки iarduino для поддержки шины I2C вашей платы.
	#endif																			//
	#endif																			//
	#endif																			//
#else																				//
...
//	Определяем тип реализации шины I2C:												//
			...
			defined(ARDUINO_ARCH_RP2040) || defined(RENESAS_CORTEX_M4)				//	Для указанных плат не поддерживается аппаратная реализация шины I2C без библиотеки Wire.
		...
		#elif defined(TwoWire_h) || defined(__ARDUINO_WIRE_IMPLEMENTATION__)		//	Проверяем не подключена ли библиотека Wire.
			#define pin_SW_SDA 255													//		№ вывода SDA не определён
			#define pin_SW_SCL 255													//		№ вывода SCL не определён
			#define iarduino_I2C_TW													//		Объявляем константу iarduino_I2C_TW				- Будет использована аппаратная реализация шины I2C под управлением библиотеки Wire.
...
//																					//
				...
				//	Если используется программная шина I2C:							//
					...
					inp_SDA		=	(volatile uint16_t*)portInputRegister(port_SDA);//	Определяем указатель на адрес регистра входных  значений для управления портом port_SDA.
					inp_SCL		=	(volatile uint16_t*)portInputRegister(port_SCL);//	Определяем указатель на адрес регистра входных  значений для управления портом port_SCL.
					#else															//
...
//				                         вход           выход						//
	...
	private:																		//
		...
		#if defined(RENESAS_CORTEX_M4)												//
		volatile uint16_t*	mod_SDA;												//	Объявляем указатель на регистр направления работы выводов порта port_SDA.
		volatile uint16_t*	mod_SCL;												//	Объявляем указатель на регистр направления работы выводов порта port_SCL.
		volatile uint16_t*	inp_SDA;												//	Объявляем указатель на регистр входных  значений порта port_SDA.
		volatile uint16_t*	inp_SCL;												//	Объявляем указатель на регистр входных  значений порта port_SCL.
		volatile uint16_t*	out_SDA;												//	Объявляем указатель на регистр выходных значений порта port_SDA.
		volatile uint16_t*	out_SCL;												//	Объявляем указатель на регистр выходных значений порта port_SCL.
		#else																		//
